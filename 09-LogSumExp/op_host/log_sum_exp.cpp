...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {   
    ...
    bool keep_dim = *context->GetAttrs()->GetBool(false);
    tiling.set_keep_dim(keep_dim); 

    /*当维度是一个列表的时候这样进行传递*/
    int attrdim_num= dim->GetSize(); 
    int dimlist[10] = {10,10,10,10,10,10,10,10,10,10}; 
    dimlist[0] = attrdim_num;
    for (int i = 0; i < attrdim_num; i++) dimlist[i]=static_cast<int>(*(dim->GetData()+i)); // 通过指针加偏移访问每个元素
    tiling.set_dimlist(dimlist);   
